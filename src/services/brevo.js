...
function getTransporter() {
  if (!transporter && BREVO_SMTP_KEY && BREVO_SMTP_KEY !== 'your_smtp_key_here') {
    transporter = nodemailer.createTransport({
      ...
      secure: false,
      auth: {
        user: BREVO_SMTP_USER || 'ecosmartai.ng@gmail.com',
        pass: BREVO_SMTP_KEY,
      },
    });
  }
...
/**
 * Send a transactional email via Nodemailer + Brevo SMTP
 */
async function sendEmail({ to, subject, htmlContent }) {
  const transport = getTransporter();

  if (!transport) {
    console.log('⚠️  Brevo SMTP not configured — email not sent');
    return null;
  }

  try {
    const info = await transport.sendMail({
      from: '"EcoSmart AI" <ecosmartai.ng@gmail.com>',
      to,
      subject,
      html: htmlContent,
    });
